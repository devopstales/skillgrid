...
================================================================================
                 ...
                 (SQLite Metadata + Filesystem Content)
================================================================================
...
- SQLite stores FILE PATHS. AI reads/writes Markdown files directly on disk.
- Link to existing skillgrid "mnemonic" memory via memory_context columns.
...
WRITE OPERATION (e.g., creating a task):
  ...
  3. If file write fails → abort (no SQL change).
  4. Insert/Update the SQLite record with the file path.
  5. If SQL insert fails → delete the orphaned file (rollback).
...
1. LEAD AGENT spawns a task:
   ...
   -> Inserts into `tasks` with `brief_path = 'files/tasks/task-001/brief.md'`
   -> Status = 'pending'

2. DEVELOPER AGENT pulls next task:
   -> SELECT * FROM tasks WHERE status='pending' ORDER BY priority LIMIT 1
   -> Reads `brief_path` from disk (raw Markdown)
   -> Works on the task (writes code, etc.)
...
3. REVIEWER AGENT performs Spec Review:
   ...
   -> Writes `files/reviews/task-001/spec_review.md`
   -> INSERT INTO reviews (passed=TRUE/FALSE, comments_path='...')
   -> If passed, UPDATE tasks SET status='review_code'
   -> If failed, UPDATE tasks SET status='in-progress' (rework)
...
5. HUMAN (optional) can manually edit any .md file in VSCode.
   -> Agent reads the updated file on next cycle.
...
- Your existing "mnemonic" memory tables (e.g., memories, embeddings) stay untouched.
- Add a `memory_context_path` column to the `tasks` table.
- When LEAD spawns a task, it queries the memory table, finds relevant context,
  writes that context to a file (or links to it), and stores the path.
- DEVELOPER agent automatically reads the memory context along with the brief,
  enabling context-aware coding without overloading the SQLite DB with large BLOBs.
...
Phase 1: Database Schema Migration
  - Alter existing skillgrid.db to add the new tables (teams, tasks, messages, etc.).
...
Phase 4: One-Time Migration (if upgrading from old system)
  - Script to extract all existing `description` fields from SQL, write them to
    `files/tasks/{task-id}/brief.md`, and update `brief_path` columns.
...
✅ SQLite handles concurrency (ACID transactions, row-level locking).
✅ Filesystem stores massive content (code diffs, long prompts) efficiently.
✅ Human-friendly: edit Markdown files directly in VSCode.
✅ Git-native: track AI's work via git diff and revert bad outputs.
✅ Lightweight MCP: only file paths are transmitted over the MCP protocol.
✅ Memory-integrated: link tasks to existing skillgrid memory via paths.
✅ Scalable: SQLite stays small; filesystem can grow indefinitely.
✅ No binary blobs: all data is plain text (Markdown) for maximum transparency.
================================================================================
