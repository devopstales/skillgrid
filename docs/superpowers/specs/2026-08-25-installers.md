# 2026-08-25 Installers Design

> **STATUS: DRAFT** — spec documents the installer requirements; implementation pending.

## Goal

Provide three installation methods for the statically-linked `skillgrid` Go CLI: a zero-dependency bash script, a Homebrew formula, and a Nix flake, all consuming GitHub Release artifacts produced by CI.
...
Before any installer can be built, the following must be in place:
...
### 1. Bash Script (`scripts/install.sh`)
...
**Tap:** `devopstales/homebrew-skillgrid`
...
**Livecheck:** Use GitHub releases to auto-detect new versions.
