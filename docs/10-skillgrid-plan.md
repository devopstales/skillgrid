...
* intent-driven-template
  ...
  * spec-as-source (BDD zone rules, opt-in)
  * gherkin-authoring — Gherkin syntax/style for spec.md
  * acceptance-test-authoring — cucumber-js / behave runner at `.skillgrid/acceptance-tests/`
  * `stack:` declared in `openspec/config.yaml` (generated by project-context)
    * [X] `acceptance-test-authoring`
    * [X] `gherkin-authoring`
* BMAD
