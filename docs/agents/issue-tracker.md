...
**Formatting** (task file templates, frontmatter schema, `Blocked by` / `Blocks` arrays, multi-component split, file placement rules): see [backlogmd-formatting.md](.agents/skills/_shared/issue-tracker/backlogmd-formatting.md).

## Conventions

- One file per ticket: `.backlog/tasks/<ID>.md`, ID assigned by the backlog CLI.
- Triage state is a `Status:` frontmatter field near the top of each file; labels configured in `backlog.config.yml`.
