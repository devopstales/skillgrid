...
- Commit messages: short imperative, one line (e.g. `fix: merge MCP entries without clobbering keys`). Never add `Co-authored-by` or other AI/agent attribution trailers — human authors only. `skillgrid install` installs a `commit-msg` hook that strips them automatically.

## Environment

- Mnemonic persistent memory is available (skillgrid local-first store: `mem_*`, `code_*`, `web_*` over the `skillgrid-mnemonic` MCP): update it on decisions, bug fixes, and non-obvious discoveries.
...
This project is indexed by GitNexus as **opensessions** (2701 symbols, 8669 relationships, 235 execution flows).
...
| Resource | Use for |
| --- | --- |
| `gitnexus://repo/opensessions/context` | Codebase overview, check index freshness |
| `gitnexus://repo/opensessions/clusters` | All functional areas |
| `gitnexus://repo/opensessions/processes` | All execution flows |
| `gitnexus://repo/opensessions/process/{name}` | Step-by-step execution trace |
...
| Tools, resources, schema reference, Mnemonic integration | `.agents/skills/gitnexus-guide/SKILL.md` |
| Memory protocol (mem_*, code_*, web_*) | `.agents/skills/mnemonic-memory/SKILL.md` |
| Memory protocol (short variant) | `.agents/skills/mnemonic-memory-protocol/SKILL.md` |
| Code index (status, search, read) | `.agents/skills/mnemonic-code-index/SKILL.md` |
