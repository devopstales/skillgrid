...
> Canonical source. All `sdd-*` skills link here (`../_shared/references/threat-matrix.md`).
> Per-skill `references/threat-matrix.md` copies are pointers to this file — do not fork content.
...
**Applicable rows are plan requirements.** They MUST propagate into `sdd-spec` as covering scenarios in the owning domain spec and into `sdd-tasks` as RED-test tasks ordered before their production tasks. An explicit `N/A` row requires no test and no scenario but MUST carry a reason a reviewer can challenge later.
...
- Every applicable row must have a `Planned RED test` entry concrete enough to become a task in `tasks.md` and a scenario in the owning spec without guessing. "A test for the edge case" is not a test. "Given `git -C` to a worktree, expect the command to fail with `not a git repository` before touching the index" is a test.
- **The handoff chain is proposal → design (marks Applicable) → spec (covering scenario per applicable row) → tasks (RED-test task ordered before its production task).** A change that loses an applicable row at any boundary was not a plan — it was a sketch.
