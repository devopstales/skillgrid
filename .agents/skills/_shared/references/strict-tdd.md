# Strict TDD Module — Apply Phase (CANONICAL)

> Canonical source. `sdd-apply` and `simple-execution` link here (`../_shared/references/strict-tdd.md`).
> `sdd-verify` audits against this module (its local `references/strict-tdd.md` is a pointer).
>
...
Based on the testing capabilities cached in Mnemonic (`sdd/{project}/testing-capabilities`), choose the appropriate test layer for each task:
