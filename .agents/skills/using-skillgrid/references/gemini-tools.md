...
Skills dispatch with `Subagent (general-purpose):` and either reference a prompt-template file (e.g., `subagent-execution`'s `references/implementer-prompt.md`) or supply an inline prompt. On Gemini CLI:
...
| References `skillgrid:requesting-code-review`'s `references/code-reviewer.md` | `invoke_agent` with `agent_name: "generalist"` and the filled review template |
