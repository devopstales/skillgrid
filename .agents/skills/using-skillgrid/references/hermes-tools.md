...
To invoke a skillgrid skill, use:

```
skill_view("skillgrid:brainstorming")
skill_view("skillgrid:test-driven-development")
```

If `skill_view` cannot find a skillgrid skill (it may not appear in the catalog
until the plugin fully registers it), fall back to reading the SKILL.md directly:

```
read_file(path="~/.hermes/plugins/skillgrid/skills/<skill-name>/SKILL.md")
```
