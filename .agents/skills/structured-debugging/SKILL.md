...
A debug session crosses compactions and the hypothesis trail must survive it —
a compaction between Phase 1 and Phase 3 that loses the trail makes you
re-investigate what you already ruled out.

- **Write the trail to a state file:** `.skillgrid/sdd/debug/<date>-<slug>/state.md`
  (git-ignored scratch, like the execution workspace — a debug session is
  ephemeral, not a committed topic). One entry per iteration:
  `hypothesis | evidence | verdict (confirmed | ruled out | open)`.
- **Append as you go** — after each Phase 2 comparison and each Phase 3
  test, before the next step. Not at the end; at the end is too late.
- **On resume:** read the state file first (skillgrid:resume); resume at the
  first `open` hypothesis. Ruled-out hypotheses stay ruled out — do not
  re-test them.
- **On root cause:** if the cause is a design decision, record an ADR
...
1. **Create Failing Test Case**
   ...
   - Use the `skillgrid:test-driven-development` skill for writing proper failing tests
...
3. **Verify Fix**
   ...
   - Use the `skillgrid:test-driven-verification` skill before claiming success
