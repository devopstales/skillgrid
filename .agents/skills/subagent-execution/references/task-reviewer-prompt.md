...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Diff file:** [DIFF_FILE]
     ...
     **Refinement (process):** the implementer reports a `Refinement:` line in its
     report (the 4-pass loop: implement → expert re-read → defect hunt → polish).
     Read it. If the diff shows areas the defect-hunt pass could not have reached
     (untouched edge cases, un-hunted integration seams) and the report claims a
     clean final pass, flag it as a Minor process note — do not block the review on
     it. Your real job is the diff, not auditing the loop.
