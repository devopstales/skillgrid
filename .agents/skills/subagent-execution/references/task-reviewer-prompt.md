...
- `[GLOBAL_CONSTRAINTS]` — the binding requirements copied verbatim from the plan's global-constraints block or the spec/design: exact values, formats, and stated relationships between components (not process rules — those are already in this template).
- `[REPORT_FILE]` — REQUIRED: the file the implementer wrote its detailed report to.
- `[BASE_SHA]` — commit before this task.
- `[HEAD_SHA]` — current commit.
- `[DIFF_FILE]` — REQUIRED: the path the controller wrote the review package to (`scripts/review-package PLAN_FILE BASE HEAD` prints the unique path; the package never enters the controller's context).
