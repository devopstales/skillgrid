...
Once a variant has won, capture the answer (which variant and why), then capture the prototype per the `Capture (when done)` section of [SKILL.md](../SKILL.md). Fold the winner into the real code and move the rest onto the throwaway branch, not into main:
