---
name: sdd-init
description: Initialize the Skillgrid SDD workspace — detect project_name, tech_stack, testing_capabilities, and issue_tracker from AGENTS.md/CLAUDE.md/GEMINI.md, openspec/config.yaml, Mnemonic, git remote, and project files; validate with the user; then build the skill registry, agent config block (AGENTS.md/CLAUDE.md/GEMINI.md), OpenSpec skeleton, Mnemonic observations, and Backlog.md if selected. Use when the user says sdd init, initialize sdd, openspec init, or sets up SDD in a new project.
---

# SDD Init

First phase of the Skillgrid SDD workflow: `init → explore → propose → design → spec → tasks → issue-creation → apply → verify → archive`. Bootstrap the SDD context so every later phase has known project facts and a working issue tracker.
...
- Source precedence for `project_name`, `tech_stack`, `testing_capabilities`, `issue_tracker`: **AGENTS.md/CLAUDE.md/GEMINI.md → openspec/config.yaml → Mnemonic → git repo → project files**. First source that answers wins; later sources fill gaps.
...
- If `openspec/` already exists, report what is there and ask before updating it.
...
```
[ ] 1. Detect existing context (AGENTS.md/CLAUDE.md/GEMINI.md, openspec/config.yaml, Mnemonic)
...
[ ] 5. Initialize persistence (git init if needed, registry, agent config, openspec, Mnemonic, optional Backlog.md)
...
- `openspec/config.yaml` — same four facts; note existing `openspec/` layout.
...
4. **OpenSpec skeleton** if absent: `openspec/config.yaml`, `openspec/specs/`, `openspec/changes/archive/`. Config format in [references/init-details.md](references/init-details.md).
5. **Mnemonic observations** — per the shared memory config in [`../_shared/conventions/mnemonic-memory.md`](../_shared/conventions/mnemonic-memory.md) (start a `mem_session_start` session first; `scope: project`):
   ...
   - `sdd/{project}/tech_stack` (type `config`) — detected tech
...
- `openspec/config.yaml` `context:` must stay under 10 lines — it is injected into every later phase.
...
- [references/init-details.md](references/init-details.md) — detection checklists, registry scan rules, OpenSpec skeleton, Mnemonic saves, output envelope.
