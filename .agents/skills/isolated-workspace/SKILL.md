...
**If NOT ignored:** Add to .gitignore, commit the change, then proceed. The `pre-commit` ignore guard (skillgrid:work-unit-commits) is a backstop: if a worktree-dir path ever gets staged, it auto-adds the dir to `.gitignore`, unstages it, and blocks the commit.
