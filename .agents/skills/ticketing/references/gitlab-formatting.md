# based on skillgrid-v2:_shared/issue-tracker/gitlab-formatting.md

# GitLab: issue formatting (epics and issues)

Shared formatting conventions for GitLab work. Consumed by the `ticketing` skill when the resolved tracker is GitLab (see `gitlab.md` for CLI conventions).
