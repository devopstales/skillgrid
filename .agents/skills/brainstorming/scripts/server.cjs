...
const SKILLGRID_VERSION = readSkillgridVersion();
const TELEMETRY_DISABLE_ENV_VARS = [
  'SKILLGRID_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SKILLGRID_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(SKILLGRID_VERSION);
  const text = SKILLGRID_TELEMETRY_DISABLED
    ? 'AISkillGrid v' + version
    : 'AISkillGrid v' + version;
  const logo = '';

  return '<div class="brand"><a href="https://github.com/devopstales/aiskillgrid">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
