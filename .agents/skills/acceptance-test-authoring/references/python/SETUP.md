...
`steps/` and `environment.py` must sit at the `.skillgrid/acceptance-tests/` root, not under a `features/` directory.
...
Page objects live in `.skillgrid/acceptance-tests/pages/`, one per screen or flow. Parse responses with BeautifulSoup; never with regexes over raw HTML. behave's `context` is the World.
