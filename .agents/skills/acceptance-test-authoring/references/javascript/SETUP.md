...
Copy into `.skillgrid/acceptance-tests/` at the repo root. Destination filenames are load-bearing.
...
`.skillgrid/acceptance-tests/` is an independent Node project with its own `package.json`. Its hooks boot the application before the suite and shut it down after, so the suite runs with one command.
...
Page objects live in `.skillgrid/acceptance-tests/support/pages/`, one per screen or flow. Parse responses with `cheerio`; never with regexes over raw HTML. The World stays a thin HTTP client holding page-object instances.
